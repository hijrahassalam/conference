  ...
  <body class=" fixed-header fixed-navigation">
	...
	<div class="row">
			...
			<div class="jarviswidget jarviswidget-color-darken" id="wid-id-0" data-widget-editbutton="false" data-widget-deletebutton="false" data-widget-colorbutton="false" data-widget-fullscreenbutton="false">
								...
								<div>
									<div class="widget-body">
										...
										<form method="POST" name="form1" id="form1" action="<?php echo base_url('sms')?>">
											<textarea name="pesan" id="pesan" class="form-control" rows="7" required autofocus maxlength="160"></textarea>
											<br/>
											<div align="left"><p id="displaycount">
											</p></div>
											<br/>
											Kirim Ke : <select name="peserta" class="select2" style="width:40%">
											<option value="all">Semua Peserta</option>
											<option value="akun">Pemilik Akun (Belum Menentukan)</option>
											<option value="pemakalah">Pemakalah</option>
											<option value="non">Non Pemakalah</option>
											<?php
											foreach ($this->db->get('registrasi')->result_array() as $row)
											{
												echo "<option value='".$row['hp']."'>".$this->account_model->nama_peserta_gelar($row['no'])." (".$row['institusi'].")</option>";
											}
											?>
											</select>
											<br/><br/>
											<input type="submit" name="send" value="Kirim SMS" class="btn btn-sm btn-primary">
										</form>
									</div>
								</div>
							</div>
		...
		$(document).ready(function() {
			...
			var area = document.getElementById("pesan");
			var message = document.getElementById("displaycount");
			var maxLength = 160;
